...
int main() {
	...
	for (auto &[dir, step] : instructions) {
		if (dir == 'R') {
			it = std::next(
			    it, std::min(step, static_cast<ll>(std::distance(it, std::prev(names.end())))));
		} else {
			it = std::prev(
			    it,
			    std::ranges::min(step, static_cast<ll>(std::ranges::distance(names.begin(), it))));
		}
	}

	std::cout << "Answer: " << *it << std::endl;
